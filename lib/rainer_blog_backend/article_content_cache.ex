defmodule RainerBlogBackend.ArticleContentCache do
  ...
  # 配置项
  @ttl_seconds Application.compile_env(:rainer_blog_backend, :content_cache_ttl, 86_400)  # 默认1天
  @stale_max_seconds Application.compile_env(:rainer_blog_backend, :content_stale_max, 604_800)  # 默认7天
  @refresh_enabled Application.compile_env(:rainer_blog_backend, :content_cache_refresh_enabled, true)
  ...
  def get_or_refresh(article_id, aws_key) do
    ...
    case get_by_article_id(article_id) do
      ...
      %__MODULE__{content: content, cached_at: cached_at} ->
        age_seconds = if cached_at, do: DateTime.diff(now, cached_at, :second), else: @stale_max_seconds + 1
        ...
        cond do
          ...
          age_seconds < @stale_max_seconds and @refresh_enabled ->
            Logger.info("Cache stale for article #{article_id}, age: #{age_seconds}s, triggering async refresh")
            # 异步刷新
            Task.start(fn ->
          ...
          # 缓存太旧或刷新已禁用，尝试同步刷新
          true ->
            Logger.warning("Cache expired for article #{article_id}, age: #{age_seconds}s, attempting sync refresh")
            case fetch_and_store(article_id, aws_key, now) do
              ...
              {:error, reason} ->
                ...
                if age_seconds < @stale_max_seconds do
                  Logger.warning("S3 fetch failed for article #{article_id}, returning stale cache: #{inspect(reason)}")
                  {:stale, content}
                else
                  Logger.error("S3 fetch failed and cache too old for article #{article_id}: #{inspect(reason)}")
                  {:error, reason}
                end
            end
        end
      ...
      {:ok, content} ->
        ...
        case upsert_by_article_id(article_id, attrs) do
          {:ok, _cache} ->
            ...
            {:ok, content}
          {:error, changeset} ->
            Logger.error("Failed to cache content for article #{article_id}: #{inspect(changeset.errors)}")
  ...
  def list_expired_caches(limit \\ 50) do
    now = DateTime.utc_now()
    threshold = DateTime.add(now, -@ttl_seconds, :second)

    from(c in __MODULE__,
      where: c.cached_at < ^threshold or is_nil(c.cached_at),
      limit: ^limit,
      order_by: [asc: c.cached_at]
    )
    |> Repo.all()
  end
  ...
  def get_stats do
    now = DateTime.utc_now()
    ttl_threshold = DateTime.add(now, -@ttl_seconds, :second)
    stale_threshold = DateTime.add(now, -@stale_max_seconds, :second)

    total = Repo.aggregate(__MODULE__, :count, :id)

    valid = from(c in __MODULE__, where: c.cached_at >= ^ttl_threshold)
            |> Repo.aggregate(:count, :id)

    stale = from(c in __MODULE__,
                 where: c.cached_at < ^ttl_threshold and c.cached_at >= ^stale_threshold)
            |> Repo.aggregate(:count, :id)

    expired = from(c in __MODULE__, where: c.cached_at < ^stale_threshold or is_nil(c.cached_at))
              |> Repo.aggregate(:count, :id)
